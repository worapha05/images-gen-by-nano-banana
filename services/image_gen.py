...
class ImageGenService:
    ...
    def generate_image(self,
        ...
        try:
            ...
            raise ImageGenServiceError(
                f"ImageGenService Error Can not process Image bytes: {str(e)}"
            )
        ...
        try:
            ...
            if image:
                ...
                raise ImageGenServiceError(
                    "ImageGenService Error No image generated from the API response"
                )
        except Exception as e:
            raise ImageGenServiceError(
                f"ImageGenService Error Can not Gen Image: {str(e)}"
            )
    ...
    async def convert_files_to_bytes(self, files: List[UploadFile]) -> List[bytes]:
        try:
            ...
            raise ImageGenServiceError(
                f"ImageGenService Error Can not convert uploaded images to bytes: {str(e)}"
            )
    ...
    def create_prompt(self, prompt: str | None, files: list[str] | None) -> str:
            if files:  
                ...
                if prompt and prompt.strip():
                    if image_count == 1:
                        final_prompt = f"Using the uploaded image as reference, create an educational illustration: {prompt.strip()} Include educational elements like classrooms, books, students, teachers, learning materials, or academic settings while maintaining the visual style of the reference image."
                    else:
                        final_prompt = f"Using the {image_count} uploaded images as references, create an educational illustration: {prompt.strip()} Combine visual styles from all images and integrate educational themes like learning environments, educational materials, students, teachers, and academic activities."
                
                else:
                    if image_count == 1:
                        final_prompt = "Transform this image into an educational context. Add educational elements like books, students, teachers, desks, whiteboards, learning materials, or classroom settings while maintaining the original style and composition."
                    else:
                        final_prompt = f"Using these {image_count} images as inspiration, create an educational illustration that combines elements from all references. Include learning environments, educational materials, students engaged in learning activities, and academic settings."
            
            else:  
                if prompt and prompt.strip():
                    final_prompt = f"Create an educational illustration: {prompt.strip()} Include learning environments (classroom, library, lab), educational materials (books, computers, supplies), and students or teachers in learning activities."
                else:
                    default_prompts = [
